...
export const userAPI = {
	...
	setUsers(page : number, count : number, term?: string) {
		return instance.get<UsersDataType>(`users?page=${page}&count=${count}&term=${term ? term : ""}`).then( response => response.data )
	}
}
...
export const friendsAPI = {
	requestFriends(page : number, count : number, term?: string) {
		return instance.get<UsersDataType>(`users?page=${page}&count=${count}&friend=true&term=${term ? term : ""}`).then( response => response.data )
	}
}